...
describe('copy destination transport', () => {
  ...
  it.each([['quizzes', duplicateQuiz], ['courses', duplicateCourse]] as const)('distinguishes omitted auto-selection from explicit global for %s', async (_kind, duplicate) => {
    const fetch = vi.fn().mockImplementation(() => Promise.resolve(new Response(JSON.stringify({ id: 'copy' }))));
    vi.stubGlobal('fetch', fetch);
    await duplicate('source');
    await duplicate('source', null);
...
describe('programme changes refresh dependent access and data', () => {
  it('covers protected data, curriculum, destination pickers and identity', () => {
    ...
    expect(prefixes).toEqual(expect.arrayContaining(['programmes', 'programme', 'courses', 'course', 'quizzes', 'quiz', 'student', 'students', 'analytics', 'user', 'batches', 'schools', 'duplicate']));
  });
});
