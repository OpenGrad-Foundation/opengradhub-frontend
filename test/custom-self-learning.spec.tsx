...
const state = vi.hoisted(() => ({ role: 'CUSTOM_LEARNER', permissions: ['scope.self', 'courses.view', 'assignments.view', 'assignments.submit', 'assessments.view', 'assessments.attempt'], quiz: vi.fn(), studentCourses: vi.fn(), catalogue: vi.fn() }));
...
vi.mock('../lib/queries/students', () => ({ useStudentCourses: (id: string) => { state.studentCourses(id); return { data: [{ id: 'enrolled', title: 'Personal enrolment', completion_percent: 0, completed_lessons: 0, total_lessons: 1 }] }; } }));
vi.mock('../lib/mutations/invalidation', () => ({ useInvalidate: () => vi.fn() }));
vi.mock('../lib/api', async original => ({
  ...(await original<typeof import('../lib/api')>()),
  getCoursesPage: (...args: unknown[]) => { state.catalogue(...args); return Promise.resolve({ items: [], total: 0, page: 1, page_size: 6, total_pages: 1 }); },
...
describe('custom self-scope learning persona', () => {
  it('keeps the extracted catalogue on personal enrolments for a custom self-scope learner', async () => {
    render(<CoursesPage />);
    expect(await screen.findByText('Personal enrolment')).toBeTruthy();
    expect(state.studentCourses).toHaveBeenCalledWith('learner');
    expect(state.catalogue).not.toHaveBeenCalled();
  });

  it('uses the scoped catalogue for a student-labelled user with staff scope', async () => {
    state.role = 'STUDENT'; state.permissions = ['scope.subtree', 'courses.view'];
    render(<CoursesPage />);
    await waitFor(() => expect(state.catalogue).toHaveBeenCalled());
    expect(screen.queryByText('Personal enrolment')).toBeNull();
    expect(state.studentCourses).toHaveBeenCalledWith('');
  });
