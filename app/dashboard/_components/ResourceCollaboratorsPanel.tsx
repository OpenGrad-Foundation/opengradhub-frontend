...
import {
  getCollaborators,
  getEligibleCollaboratorsFor,
  type CollabResource,
  type CourseCollaborator,
  type EligibleCollaborator,
} from "@/lib/api";
import {
  useAddResourceCollaborator,
  useRemoveResourceCollaborator,
} from "@/lib/mutations/collaborators";

/**
 * Generic collaborators panel for batches / quizzes / assignments — same UX
 * as the course collaborators panel (course-management/[id]/collaborators-panel.tsx).
 * The list is visible to any manager of the resource; add/remove controls only
 * to the creator or a SUPER_ADMIN (the backend enforces the same rule).
 */
export default function ResourceCollaboratorsPanel({
  resource,
  resourceId,
  createdBy,
  callerId,
  ...
  const addMutation = useAddResourceCollaborator(resource);
  const removeMutation = useRemoveResourceCollaborator(resource);
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const list = await getCollaborators(resource, resourceId);
      setCollaborators(list);
      if (canShare) setEligible(await getEligibleCollaboratorsFor(resource, resourceId));
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load collaborators.");
    } finally {
      setLoading(false);
    }
  }, [resource, resourceId, canShare]);
  ...
  const handleAdd = async (userId: string) => {
    try {
      setError(null);
      await addMutation.mutateAsync({ id: resourceId, userId });
      setQuery("");
      setPickerOpen(false);
      await load();
    } catch (e) {
  ...
  const handleRemove = async (userId: string) => {
    try {
      setError(null);
      await removeMutation.mutateAsync({ id: resourceId, userId });
      await load();
    } catch (e) {
