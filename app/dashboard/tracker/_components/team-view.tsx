...
export function TeamView({ onOpen, onAssign, initialOwnerId }: {
  ...
  const crumbs = (
    <nav aria-label="Team path" className="flex flex-wrap items-center gap-1 text-sm">
      <button type="button" onClick={() => goTo(0)} className={"font-medium " + (path.length === 0 && !person ? "text-[var(--color-text)]" : "text-[var(--teal)] hover:underline")}>Your team</button>
      {path.map((p, i) => (
        <span key={p.id} className="flex items-center gap-1">
          <ChevronRight className="h-3.5 w-3.5 text-[var(--color-text-muted)]" aria-hidden="true" />
          <button type="button" onClick={() => goTo(i + 1)} className={"font-medium " + (i === path.length - 1 && !person ? "text-[var(--color-text)]" : "text-[var(--teal)] hover:underline")}>{p.name}</button>
        </span>
      ))}
      {person && (
        <span className="flex items-center gap-1">
          <ChevronRight className="h-3.5 w-3.5 text-[var(--color-text-muted)]" aria-hidden="true" />
          <span className="font-medium text-[var(--color-text)]">{person.name}</span>
        </span>
      )}
    </nav>
  );
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-sm font-semibold text-[var(--color-text)]">{person.name}&apos;s tasks</h3>
    ...
    <div className="flex flex-col gap-4">
      ...
      {current && (
        <div>
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <h3 className="text-sm font-semibold text-[var(--color-text)]">{current.name}&apos;s own tasks</h3>
      ...
      <section className="overflow-hidden rounded-xl border border-[var(--color-border)] bg-white">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border)] px-4 py-3">
          <h3 className="text-base font-semibold text-[var(--color-text)]">{current ? `Reports to ${current.name}` : "Your team"}</h3>
          <div className="flex flex-wrap items-center gap-2">
            {roles.length > 1 && (
              <label className="flex items-center gap-1.5 text-xs font-medium text-[var(--color-text-muted)]">Role
                <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="h-9 rounded-lg border border-[var(--color-border-strong)] bg-white px-2 text-sm text-[var(--color-text)] outline-none focus:border-[var(--teal)]">
                  <option value="">All</option>
                  {roles.map((r) => <option key={r} value={r}>{roleLabel(r)}</option>)}
                </select>
              </label>
            )}
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search team…" className="h-9 w-48 rounded-lg border border-[var(--color-border-strong)] bg-white px-3 text-sm outline-none focus:border-[var(--teal)]" />
          </div>
        </div>
        {level.isLoading ? (
          ...
          <p className="px-4 py-8 text-center text-sm text-[var(--color-text-muted)]">{current ? `Nobody reports to ${current.name}.` : "Staff you manage will appear here."}</p>
        ) : shown.length === 0 ? (
          <p className="px-4 py-8 text-sm text-[var(--color-text-muted)]">No team members match.</p>
        ) : (
          <ul className="divide-y divide-[var(--color-border)]">
            {shown.map((m) => (
              <li key={m.id} className="flex items-center gap-2 px-1">
                <button type="button" onClick={() => openRow(m)}
                  className="group flex w-full cursor-pointer items-center justify-between gap-3 rounded-lg px-3 py-3.5 text-left text-sm transition-colors hover:bg-[var(--color-success-surface)]">
                  <span className="flex min-w-0 items-center gap-2 font-medium text-[var(--color-text)]">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-100 text-xs font-semibold text-[var(--teal)]">
                  ...
                  <span className="flex shrink-0 items-center gap-3">
                    <span className="hidden items-center gap-2 text-xs sm:flex">
                      {m.report_count > 0 && (<><span className="text-[var(--color-text-muted)]">{m.report_count} report{m.report_count === 1 ? "" : "s"}</span><span className="text-gray-300">·</span></>)}
                    ...
                    <span className="inline-flex items-center gap-1 rounded-lg border border-teal-200 bg-[var(--color-success-surface)] px-2.5 py-1 text-xs font-semibold text-[var(--teal)] transition group-hover:bg-teal-100">
                ...
                {canAuthor && onAssign && canReceiveTask(m.role) && (
                  <button type="button" onClick={() => onAssign({ id: m.id, name: m.name })} aria-label={`Assign task to ${m.name}`} title="Assign task"
                    className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[var(--color-border-strong)] bg-white text-[var(--color-text-muted)] transition hover:border-teal-400 hover:text-[var(--teal)]">
...
export function AssignTaskButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-lg bg-[var(--green)] px-3 py-1.5 text-sm font-semibold text-[var(--dark-teal)] transition hover:bg-[var(--green)]"
    >
