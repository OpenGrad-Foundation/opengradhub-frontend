...
export function AllTasksPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {isLoading ? (
        <div className="flex min-h-40 items-center justify-center rounded-xl border border-[var(--color-border)] bg-white"><Loader2 className="h-5 w-5 animate-spin text-[var(--teal)]" aria-hidden="true" /></div>
      ) : error ? (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr
                    ...
                    className="cursor-pointer border-t border-[var(--color-border)] transition-colors hover:bg-[var(--color-success-surface)]/50"
                  >
                    <td className="px-4 py-3 font-medium text-[var(--color-text)]">{r.name}</td>
                    <td className="px-4 py-3 text-[var(--color-text-muted)]">{TARGET_LABEL[r.target_type]}</td>
                    <td className="px-4 py-3 capitalize text-[var(--color-text-muted)]">{r.priority}</td>
                    <td className="px-4 py-3 text-[var(--color-text)]">
                      <ProgressBar done={r.done} total={r.total} />
                    </td>
                    <td className="px-4 py-3 text-[var(--color-text)]">{r.total}</td>
                    <td className="px-4 py-3"><StatePill state={r.rolled_state} /></td>
                    <td className="px-4 py-3 text-[var(--color-text-muted)]">{r.deadline ? formatDate(r.deadline) : "—"}</td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {onOpenTask && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded border border-[var(--color-border)] bg-white px-2.5 py-1 text-xs font-semibold text-[var(--teal)] transition hover:border-teal-300 hover:bg-[var(--color-success-surface)]"
                          >
      ...
      <div className="flex items-center justify-between text-sm text-[var(--color-text-muted)]">
        ...
        <div className="flex items-center gap-2">
          <button type="button" disabled={page <= 1} onClick={() => setPage((n) => n - 1)} className="rounded-lg border border-[var(--color-border-strong)] px-2.5 py-1 disabled:opacity-40">Prev</button>
          <span>Page {page} / {pages}</span>
          <button type="button" disabled={page >= pages} onClick={() => setPage((n) => n + 1)} className="rounded-lg border border-[var(--color-border-strong)] px-2.5 py-1 disabled:opacity-40">Next</button>
        </div>
      </div>
    </div>
  );
...
function ProgressBar({ done, total }: { done: number; total: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-2 w-24 overflow-hidden rounded-full bg-[#eef5f3]">
        <div className="h-full rounded-full bg-emerald-500" style={{ width: `${pct}%` }} />
      </div>
      <span className="whitespace-nowrap text-xs text-[var(--color-text-muted)]">{done}/{total} done</span>
    </div>
  );
}
