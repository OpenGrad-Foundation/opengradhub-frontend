...
export function StudentDetailsBulkUpload({ onClose }: { onClose: () => void }) {
  ...
  return <Modal title={<h2 className="text-lg font-semibold text-gray-950">Bulk upload student details</h2>} onClose={close} maxWidth="1000px">
    <div className="flex max-h-[75dvh] min-h-0 flex-col gap-4">
      ...
      <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-gray-100 pt-3">
...
function CsvPreview({ csv }: { csv: StudentCsv }) {
  const [page, setPage] = useState(0);
  return <><div tabIndex={0} role="region" aria-label="CSV data preview" className="max-h-72 overflow-auto rounded-md border border-gray-200">
    <table className="w-full text-left text-xs"><thead className="sticky top-0 bg-gray-50"><tr><th className="p-2">Row</th>{csv.headers.map(h => <th key={h} className="whitespace-nowrap p-2">{h}</th>)}</tr></thead>
      <tbody>{csv.rows.slice(page * 25, (page + 1) * 25).map(row => <tr key={row.row_number} className="border-t border-gray-100"><td className="p-2 text-gray-500">{row.row_number}</td>{row.cells.map((cell, i) => <td key={i} className="max-w-64 whitespace-pre-wrap break-words p-2">{cell}</td>)}</tr>)}</tbody></table>
...
function PreviewPages({ page, setPage, total, label, disabled }: { page: number; setPage: (page: number) => void; total: number; label: string; disabled?: boolean }) {
  ...
  return <div className="flex items-center justify-between gap-2 pt-2 text-xs text-gray-600">
