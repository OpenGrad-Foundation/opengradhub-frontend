...
/** Above this, re-encode once more at a lower quality before giving up on shrinking. */
const RETRY_OVER_BYTES = 600 * 1024;

/** Downscale a captured image to a max edge and re-encode as JPEG to keep field uploads
 *  small. This is a bandwidth optimisation only — the server re-encodes every proof
 *  photo — so it fails OPEN: on any decode/canvas error, send the original file rather
 *  than blocking the upload (some browsers can't decode e.g. HEIC into an ImageBitmap). */
export async function downscaleImage(file: File, maxEdge = 1280): Promise<Blob> {
  let bitmap: ImageBitmap | null = null;
  try {
    bitmap = await createImageBitmap(file);
    const scale = Math.min(1, maxEdge / Math.max(bitmap.width, bitmap.height));
    // A panorama can round its short edge to zero, which throws on drawImage.
    ...
    // A JPEG has no alpha channel: without this, transparent pixels encode as black.
    ...
    const blob = await encode(0.72);
    if (blob && blob.size > RETRY_OVER_BYTES) return (await encode(0.55)) ?? blob;
    return blob ?? file;
  } catch {
